...
# =====================================================================
# חלק 2: מנוע חישוב תשואות קשיח מתחילת החודש הנוכחי (Month-to-Date)
# =====================================================================
...
def get_benchmark_returns():
    ...
    # חישוב דינמי של ה-1 לחודש הנוכחי
    today = datetime.now()
    start_of_month = datetime(today.year, today.month, 1).strftime('%Y-%m-%d')
    
    # 1. משיכת תשואות המדדים הרגילים מתחילת החודש
    for name, ticker in BENCHMARKS.items():
        try:
            # משיכה מבוססת תאריך התחלה קשיח למניעת עיוותים בין חודשים
    ...
    # 2. חישוב השינוי בשער הדולר מול השקל (USD/ILS) בדיוק מתחילת החודש
    usd_effect = 0.0
    try:
    ...
    # 3. החלת אפקט המט"ח המגודר חלקי לפי בחירת החברה (מתוך ה-Session State)
...
def get_historical_tracks_returns(chosen_tracks, available_tracks):
    ...
    בבלוקים חודשיים קבועים של 21 ימי מסחר.
    """
    data_list = []
    cached_histories = {}
...
# עדכון בר ההתקדמות העליון לפי העמוד הנוכחי
