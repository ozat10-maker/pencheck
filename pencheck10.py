...
# =====================================================================
# מנגנון בסיס נתונים מקומי (JSON) לניהול משתמשים ושמירת נתונים
# =====================================================================
...
if "pension_page" not in st.session_state:
    st.session_state.pension_page = "login"  # דף ברירת המחדל החדש
...
# הגדרות עיצוב ומפתח AI בתפריט הצד
st.set_page_config(page_title="מערכת AI לפנסיה עם אזור אישי", page_icon="🔐", layout="wide")
...
COMPANY_TRACKS_REGISTRY = {
    ...
    "מגדל מקפת": {
        "מגדל מקפת מנייתי": {"components": {"S&P 500": 45, "TA 125": 20, "Nasdaq 100": 20, "Bonds": 10, "Cash": 5}, "default_fx": 35.0},
...
# =====================================================================
# חלק 2: מנוע חישוב תשואות חודשי קשיח והפקת שמות חודשים בעברית
# =====================================================================
...
def get_historical_tracks_returns(chosen_tracks, available_tracks):
    ...
    for i in range(1, 4):
        ...
        # חישוב דינמי של חודש ושנת העבר בעברית
...
# ניהול הצגת תפריט התנתקות במידה והמשתמש מחובר למערכת
...
st.write("---")
# =====================================================================
# חלק 3: מסכי התחברות (Login), רישום (Register) ושלב 1 (פרטי החוסך)
# =====================================================================
...
if st.session_state.pension_page == "login":
    st.title("🔐 כניסה למערכת AI לפנסיה")
    ...
    if col1.button("התחבר", type="primary"):
        ...
        if username in db and db[username]["password"] == hash_password(password):
            ...
            # טעינת נתונים פיננסיים אחרונים שנשמרו למשתמש זה
...
# ---------------------------------------------------------------------
    ...
    st.title("📝 הרשמה למערכת AI לפנסיה")
...
# ---------------------------------------------------------------------
# שלב 1: הגדרת נתוני החוסך (עם טעינה אוטומטית)
# ---------------------------------------------------------------------
    ...
    st.write(f"שלום **{st.session_state.logged_in_user}**, אנא עדכן את נתוני הפנסיה שלך:")
    ...
    if st.button("המשך לבחירת מסלולי ההשקעה", type="primary"):
        ...
        navigate_to("page2")
# =====================================================================
# חלק 4: שלב 2 (איזון מסלולים ושמירת נתונים קבועה) ותחילת שלב 3
# =====================================================================
    ...
    if c_next.button("בצע ניתוח ביצועים ודוח AI! 🚀", type="primary", disabled=(total_split_pct != 100)):
        ...
        # ---------------------------------------------------------------------
        # מנגנון שמירת הנתונים האחרונים של המשתמש לבסיס הנתונים (JSON)
        # ---------------------------------------------------------------------
        db = load_db()
...
# =====================================================================
# חלק 5: שלב 3 (המשך תצוגה וטבלה בעברית) ושלב 4 (סימולציית פרישה)
# =====================================================================

    # תצוגת המטריקות לחודש הנוכחי (מאי 2026)
    ...
    try:
        # פונקציה זו מחשבת ומציגה כעת את שמות החודשים האמיתיים בעברית (למשל: אפריל 2026, מרץ 2026 וכו')
    ...
    if not api_key: 
        ...
        system_instruction = "אתה מומחה פנסיוני ואקטואר בכיר. נתח את ביצועי החודש וההיסטוריה בהתחשב בחשיפת המט\"ח והגידור החודשי הקשיח, וספק דוח מקצועי בעברית בלבד."
...
# ---------------------------------------------------------------------
# דף 4: סימולציית הון וקצבה צפויה בגיל 65
# ---------------------------------------------------------------------
