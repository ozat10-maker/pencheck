...
# Mapping institutional companies to their official investment tracks
...
if st.session_state.pension_page == "page1":
    ...
    if st.button("המשך לבחירת מסלולי ההשקעה ➡️", type="primary"):
        ...
        navigate_to("page2")
# =====================================================================
# 📊 דף 2: הגדרת חלוקת מסלולים משולבת ומרובה (משודרג)
# =====================================================================
elif st.session_state.pension_page == "page2":
    ...
    # Enable picking multiple investment tracks simultaneously via Multi-Select
    chosen_tracks = st.multiselect("בחר את מסלולי ההשקעה הפעילים בקופה שלך:", list(available_tracks.keys()), default=[list(available_tracks.keys())[0]])
    ...
    with col1:
        ...
        if not chosen_tracks:
            ...
            # Dynamically generate weight inputs based on chosen tracks
            for track in chosen_tracks:
                # Default uniform weight split
    ...
    # Mathematically aggregate asset profiles based on portfolio track weights
    ...
    if total_split_pct == 100 and chosen_tracks:
        for track, track_weight in track_split_data.items():
            track_profile = available_tracks[track]
    ...
    with col2:
        ...
        if total_split_pct == 100:
            ...
            fig = px.pie(mix_df, values="Aחוז" if "Aחוז" in mix_df.columns else "אחוז", names="אפיק השקעה", hole=0.4, title="פילוח משוקלל משולב")
    ...
    if c_next.button("בצע ניתוח ביצועים ודוח AI! 🚀", type="primary", disabled=(total_split_pct != 100)):
        # Construct summary string of split tracks for the AI report context
...
# =====================================================================
    ...
    st.subheader(f"אומדן ביצועים עבור שילוב המסלולים: {u['fund']} בחברת {u['company']}")
    ...
    with st.spinner("שולף נתוני שוק ומחשב עלויות דמי ניהול..."):
        ...
        if not api_key:
            ...
            with st.spinner("מנוע ה-AI מנתח את נתוני הפנסיה המשולבים שלך..."):
                user_context = f"Company: {u['company']}, Split Tracks Profile: {u['fund']}, Age: {u['age']}, Balance: {u['balance']} NIS, Monthly Fees: {total_monthly_fees_nis} NIS, Calculated Combined Net Return: {total_net_return}%"
                system_instruction = "אתה מומחה פנסיוני ואקטואר בכיר בשוק ההון הישראלי. נתח את הנתונים וספק דוח מקצועי בעברית בלבד הכולל ביקורת דמי ניהול והתאמת גיל למבנה המסלולים המשולב."
                try:
