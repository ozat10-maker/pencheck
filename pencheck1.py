...
def get_benchmark_returns():
    benchmarks = {
        "מניות חו"ל (S&P 500)": "^SPX",
        "מניות ישראל (ת"א 125)": "^TA125.TA",
        "טכנולוגיה (Nasdaq 100)": "^NDX",
        "אג"ח ממשלתי/כללי": "AGG",
        "מזומן/נכסים לא סחירים": "BIL"
    }
...
if st.session_state.pension_page == "page1":
    ...
    # טעינת ערכי ברירת מחדל מה-State אם המשתמש חזר אחורה מדף 2
    ...
    st.write("---")
    # שמירה ל-State ומעבר לדף הבא באמצעות פונקציית הניווט
...
# =====================================================================
    ...
    # הגנה: אם משתמש מנסה לגשת לדף 2 בלי להזין נתוני יסוד, נחזיר אותו אוטומטית לדף 1
    ...
    with col1:
        ...
        p_sp500 = st.slider("מניות חו"ל (S&P 500) - %", 0, 100, saved_mix.get("מניות חו\"ל (S&P 500)", 50))
        p_tlv = st.slider("מניות ישראל (ת"א 125) - %", 0, 100, saved_mix.get("מניות ישראל (ת\"א 125)", 20))
        p_nasdaq = st.slider("טכנולוגיה (Nasdaq 100) - %", 0, 100, saved_mix.get("טכנולוגיה (Nasdaq 100)", 10))
        p_bonds = st.slider("איגרות חוב (אג\"ח ממשלתי/קונצרני) - %", 0, 100, saved_mix.get("אג\"ח ממשלתי/כללי", 15))
        p_cash = st.slider("מזומן ונכסים לא סחירים - %", 0, 100, saved_mix.get("מזומן/נכסים לא סחירים", 5))
    ...
    with col2:
        ...
        mix_df = pd.DataFrame({
            "אפיק השקעה": ["S&P 500", "ת"א 125", "Nasdaq 100", "אג\"ח", "מזומן/אחר"],
            "אחוז": [p_sp500, p_tlv, p_nasdaq, p_bonds, p_cash]
        })
        fig = px.pie(mix_df, values="אחוז", names="אפיק השקעה", hole=0.4, title="תמהיל נכסי הפנסיה שהוגדר")
    ...
    if c_next.button("בצע ניתוח ביצועים ודוח AI! 🚀", type="primary", disabled=(total_pct != 100)):
        st.session_state.mix_data = {
            "מניות חו"ל (S&P 500)": p_sp500,
            "מניות ישראל (ת"א 125)": p_tlv,
            "טכנולוגיה (Nasdaq 100)": p_nasdaq,
            "אג\"ח ממשלתי/כללי": p_bonds,
            "מזומן/נכסים לא סחירים": p_cash
        }
...
# =====================================================================
    ...
    with st.spinner("שולף נתוני שוק ומחשב עלויות דמי ניהול..."):
        ...
        # 1. חישוב תשואה משוקללת ברוטו לקופה
        ...
        # 2. חישוב דמי ניהול חודשיים בשקלים ונטו
        ...
        # הפקת דוח מומחה בעזרת Gemini AI
        st.write("---")
        ...
        if not api_key:
            ...
            with st.spinner("מנוע ה-AI מנתח את נתוני הפנסיה ודמי הניהול שלך..."):
                total_stocks = st.session_state.mix_data["מניות חו"ל (S&P 500)"] + st.session_state.mix_data["מניות ישראל (ת"א 125)"] + st.session_state.mix_data["טכנולוגיה (Nasdaq 100)"]
