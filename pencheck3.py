...
# =====================================================================
    ...
    with st.spinner("שולף נתוני שוק ומחשב עלויות דמי ניהול..."):
        ...
        st.session_state.calculated_net_return = total_net_return
        st.session_state.total_monthly_fees_nis = total_monthly_fees_nis
...
# =====================================================================
# 📈 שלב 4: סימולציית פרישה, חישוב צבירה וקצבה חודשית
# =====================================================================
elif st.session_state.pension_page == "projection":
    if not st.session_state.user_info:
        navigate_to("page1")
    ...
    st.write("מנוע זה מחשב את צמיחת הקופה ברמה שנתית (ריבית דריבית) על בסיס הנתונים שהזנת.")
    ...
    if years_to_retire <= 0:
        ...
        st.write("כדי לבצע חישוב ארוך טווח, עלינו להניח מה תהיה התשואה השנתית הממוצעת של הקופה עד הפרישה:")
        ...
        annual_return_input = st.number_input("הנחת תשואה שנתית ממוצעת לקופה (%):", min_value=1.0, max_value=15.0, value=suggested_annual_return, step=0.5)
        ...
        net_annual_deposit = annual_deposit * (1 - (u["fee_deposit"] / 100))
        ...
        st.write("---")
        st.subheader("📊 השורה התחתונה בגיל 65")
        
        p1, p2, p3 = st.columns(3)
        p1.metric("שנים נותרות לפרישה", f"{years_to_retire} שנים", f"מגיל {current_age} עד גיל 65")
        p2.metric("סכום צבורה משוער בפרישה", f"{final_balance:,.0f} ₪")
        p3.metric("קצבה חודשית צפויה", f"{estimated_pension:,.0f} ₪ / לחודש", f"לפי מקדם המרה של {conversion_coefficient}")
        
        st.write("### 📉 גרף התפתחות הון הפנסיה לאורך השנים")
        chart_df = pd.DataFrame({
            "גיל החוסך": age_axis,
            "שווי הקופה הצפוי (₪)": balance_axis
        })
        fig_line = px.line(chart_df, x="גיל החוסך", y="שווי הקופה הצפוי (₪)", 
                           title="צמיחת חסכון הפנסיה המצטבר (כולל ריבית דריבית)", markers=True)
